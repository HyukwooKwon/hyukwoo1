...
상대 `RunRoot` 경로는 현재 작업 디렉터리가 아니라 repo root 기준으로 해석합니다. 다른 cwd에서 실행해도 `.\pair-test\...`는 항상 repo 아래를 가리키는 것이 기준입니다.
...
panel을 사용 중이면 홈 탭과 운영 탭의 `runroot 요약` 버튼으로 같은 helper를 바로 실행할 수 있습니다.
...
- `SchemaVersion`, `GeneratedAt`, `PairDefinitionSource`, `Warnings`, `ConfigHash`
...
pair01만 바로 돌리려면 panel의 `pair01 즉시 실행` 버튼을 사용합니다. 이 버튼은 현재 visible lane 설정을 고정해서 `target01 -> target05 -> target01` 한 번 왕복을 실행하고, preview 산출물도 같이 저장합니다.

Windows 더블클릭 원클릭 실행:

- [open-run-pair01-headless-drill.vbs](C:\dev\python\hyukwoo\hyukwoo1\open-run-pair01-headless-drill.vbs)
- 내부 래퍼: [launch-run-pair01-headless-drill.cmd](C:\dev\python\hyukwoo\hyukwoo1\launch-run-pair01-headless-drill.cmd)
- 실제 실행: [run-pair01-headless-drill.ps1](C:\dev\python\hyukwoo\hyukwoo1\run-pair01-headless-drill.ps1)

이 경로는 `config\settings.bottest-live-visible.psd1`와 `pair01`, `target01` 시작을 고정해서 바로 테스트를 돌립니다.
...
각 pair의 top target은 `target01`, `target02`, `target03`, `target04`로 고정하고, 대응 bottom target은 `target05`, `target06`, `target07`, `target08`입니다.
