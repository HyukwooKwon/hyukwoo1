...
Assert-True ([string]$result.WorkflowKind -eq 'pair-visible-acceptance') 'preflight-only receipt should identify the pair-visible-acceptance workflow.'
Assert-True ([string]$result.WorkflowScope -eq 'pair') 'preflight-only receipt should identify pair workflow scope.'
Assert-True (-not [bool]$result.TargetAutoloopEquivalent) 'preflight-only receipt should explicitly state it is not TargetAutoloop equivalent.'
Assert-True ([string]$result.OperatorScopeWarning -like '*TargetAutoloop*') 'preflight-only receipt should warn operators this is not independent-cell TargetAutoloop.'
