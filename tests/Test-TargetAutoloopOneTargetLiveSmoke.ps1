...
$routerInboxRoot = Join-Path $externalWorkRoot 'router-inbox\target04'
$runtimeRoot = Join-Path $externalWorkRoot 'runtime'
...
@(
    [ordered]@{
        TargetId = 'target04'
        LauncherSessionId = 'test-session-live-smoke'
    }
...
@{
    ...
    Targets = @(
        @{ Id = 'target04'; Folder = '$($routerInboxRoot.Replace("'", "''"))'; WindowTitle = 'Target04'; FixedSuffix = 'suffix-live-smoke' }
    )
    TargetAutoloop = @{
        Enabled = `$true
        ...
        Targets = @(
            @{ TargetId = 'target04'; Enabled = `$true; TriggerKinds = @('input-file', 'publish-ready'); MaxCycleCount = 2 }
        )
    }
}
...
Assert-True ([string]$target04.SourceSummaryPath -like 'C:\dev\python\relay-target-autoloop-live-smoke*') 'source summary contract path should stay outside the automation repo.'
Assert-True ([string]$target04.SourceReviewZipPath -like 'C:\dev\python\relay-target-autoloop-live-smoke*') 'source review zip contract path should stay outside the automation repo.'
Assert-True ([string]$target04.PublishReadyPath -like 'C:\dev\python\relay-target-autoloop-live-smoke*') 'publish-ready contract path should stay outside the automation repo.'

$inputPath = Join-Path $target04.InboxPendingRoot 'task_live_smoke_001.txt'
Set-Content -LiteralPath $inputPath -Encoding UTF8 -Value 'single target live smoke cycle 1 input'

$cycle1WatchJson = & pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'tests\Watch-TargetAutoloop.ps1') `
    -ConfigPath $configPath `
    -RunRoot $runRoot `
...
$completedFilesAfterCycle1 = @(Get-ChildItem -LiteralPath $target04.QueueCompletedRoot -File -Filter '*.json' | Sort-Object Name)
$stateAfterCycle1 = Get-Content -LiteralPath $start.StatePath -Raw -Encoding UTF8 | ConvertFrom-Json
$targetAfterCycle1 = $stateAfterCycle1.Targets.target04
...
Set-Content -LiteralPath $target04.SourceSummaryPath -Encoding UTF8 -Value 'cycle 1 output summary ready for next target-autoloop cycle'
$cycle1ZipNotePath = Join-Path $target04.SourceOutboxPath 'cycle1_note.txt'
Set-Content -LiteralPath $cycle1ZipNotePath -Encoding UTF8 -Value 'cycle 1 zip payload'
if (Test-Path -LiteralPath $target04.SourceReviewZipPath) {
    Remove-Item -LiteralPath $target04.SourceReviewZipPath -Force
}
Compress-Archive -LiteralPath $cycle1ZipNotePath -DestinationPath $target04.SourceReviewZipPath -Force
$publish1Json = & pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'tests\Publish-TargetAutoloopArtifact.ps1') `
    -ConfigPath $configPath `
    -RunRoot $runRoot `
    -TargetId target04 `
    -CycleId 1 `
...
$cycle2WatchJson = & pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'tests\Watch-TargetAutoloop.ps1') `
    -ConfigPath $configPath `
    -RunRoot $runRoot `
    -DispatchQueuedCommandsInline `
    -ProcessOnce `
...
$completedFilesAfterCycle2 = @(Get-ChildItem -LiteralPath $target04.QueueCompletedRoot -File -Filter '*.json' | Sort-Object Name)
$stateAfterCycle2 = Get-Content -LiteralPath $start.StatePath -Raw -Encoding UTF8 | ConvertFrom-Json
$targetAfterCycle2 = $stateAfterCycle2.Targets.target04
...
Set-Content -LiteralPath $target04.SourceSummaryPath -Encoding UTF8 -Value 'cycle 2 output summary should not trigger cycle 3 because max cycle count is reached'
$cycle2ZipNotePath = Join-Path $target04.SourceOutboxPath 'cycle2_note.txt'
Set-Content -LiteralPath $cycle2ZipNotePath -Encoding UTF8 -Value 'cycle 2 zip payload'
if (Test-Path -LiteralPath $target04.SourceReviewZipPath) {
    Remove-Item -LiteralPath $target04.SourceReviewZipPath -Force
}
Compress-Archive -LiteralPath $cycle2ZipNotePath -DestinationPath $target04.SourceReviewZipPath -Force
$publish2Json = & pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'tests\Publish-TargetAutoloopArtifact.ps1') `
    -ConfigPath $configPath `
    -RunRoot $runRoot `
    -TargetId target04 `
    -CycleId 2 `
...
$closeoutWatchJson = & pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'tests\Watch-TargetAutoloop.ps1') `
    -ConfigPath $configPath `
    -RunRoot $runRoot `
    -DispatchQueuedCommandsInline `
    -ProcessOnce `
...
$finalStatusText = & pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'tests\Show-TargetAutoloopStatus.ps1') `
    -ConfigPath $configPath `
    -RunRoot $runRoot
$finalState = Get-Content -LiteralPath $start.StatePath -Raw -Encoding UTF8 | ConvertFrom-Json
$finalTarget = $finalState.Targets.target04
$queuedFilesFinal = @(Get-ChildItem -LiteralPath $target04.QueueQueuedRoot -File -Filter '*.json' | Sort-Object Name)
...
Assert-True ([string]$finalStatus.SmokeReceipt.TargetId -eq 'target04') 'final status json should surface the smoke receipt target.'
...
Assert-True ($finalStatusText -match 'SmokeSummary: smoke: passed / proof=script-level / source=script-smoke / target=target04 / cycle=2/2 / phase=limit-reached / stop=all-targets-limit-reached') 'final status text should surface the compact smoke summary.'
